...
def est_clique(Chi, delta):
    ...
    """
    # Create a complementary matrix where elements greater than or equal to delta are 1, otherwise 0
    complementary_matrix = (Chi >= delta) * 1.0
...
def est_pure_set(Chi, delta):
    ...
    clique_max = est_clique(Chi, delta)
    ...
    for i in clique_max:
        # Find indices where the condition 1 - Chi[i, :] < delta holds
        index = np.where(1 - Chi[i, :] < delta)[0]
